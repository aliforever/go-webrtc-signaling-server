...
func server(listener *webrtcsignalingserver.Listener, wg *sync.WaitGroup) {
	...
	fmt.Println("client SDP is:", clientSDP, "with data", data)

	fmt.Println("Writing server SDP")
...
func client(listener *webrtcsignalingserver.Listener, wg *sync.WaitGroup) {
	...
	fmt.Println("Writing Client SDP")
	...
	serverSDP, data := listener.ReadServerSDP()
	fmt.Println("Received Server SDP", serverSDP, "with data", data)
}
